...
plot_densityHeatmap <- function(df, title){
    return(
   ggplotify::as.ggplot(densityHeatmap(df %>% scale , ylim = c(-2, 2), cluster_columns = TRUE, title = title, ylab = "",
...
AAs_Control_plot <-AAs_Control  %>% plot_densityHeatmap("")
ACs_Control_plot <-ACs_Control   %>% plot_densityHeatmap("")
AAs_PKU_plot <- AAs_PKU   %>% plot_densityHeatmap("")
ACs_PKU_plot <- ACs_PKU   %>% plot_densityHeatmap("")
```


```{r fig.height=4, fig.width=7, message=FALSE, warning=FALSE}
densityHeatmap_panel <- ggarrange(ACs_Control_plot,ACs_PKU_plot,AAs_Control_plot,AAs_PKU_plot, ncol = 1 ,  common.legend = T, heights = c(1,1,.9,.9))
...
UMAP_plus_graph <- ggarrange(UMAP_reduction, graph, ncol = 1, heights = c(.7, 1))
